...
class HeatmapGenerator ():
    ...
    def __init__ (self, pathModel, nnArchitecture, nnClassCount, transCrop):
        ...
        if nnArchitecture == 'DENSE-NET-121': model = DenseNet121(nnClassCount, True).to(device)
        elif nnArchitecture == 'DENSE-NET-169': model = DenseNet169(nnClassCount, True).to(device)
        elif nnArchitecture == 'DENSE-NET-201': model = DenseNet201(nnClassCount, True).to(device)
